...
```
  ...
  async _makeRequest(opts, originalCall = true) {
    ...
    // API ISSUE NOTE: It turns out that sometimes the Litmos API will actually return _more_ items
    // than requested. This needs to be accounted for
    if (!usingCustomLimit && res.body.length >= originalOpts.params.limit) {
      originalOpts.params.start += res.body.length;
